/* ═══════════════════════════════════════════
   SUGARCLOSET — Interactive Script
   ═══════════════════════════════════════════ */
    ...
    document.getElementById('modalBody').innerHTML = `
        ...
        <div class="modal-info">
            ...
            <div style="margin-top: 12px;">
                <a href="https://wa.me/${SHOP_INFO.phone}?text=${encodeURIComponent('Halo kak, saya mau tanya tentang: ' + p.name + ' (' + formatRupiah(p.price) + ')')}" 
                   target="_blank" 
...
const toggleFavFromModal = (id) => {
    ...
    updateBadges();
    renderProducts();
    openProduct(id); // refresh modal
};

const saveFavs = () => {
    localStorage.setItem('sugarcloset_favs', JSON.stringify(favorites));
};
...
const saveCart = () => {
    localStorage.setItem('sugarcloset_cart', JSON.stringify(cart));
};
...
const renderCart = () => {
    ...
    container.innerHTML = cart.map(item => {
        ...
        return `
            <div class="cart-item">
                <div class="cart-item-img" style="background: ${p.color}60;">${p.image}</div>
...
const checkout = () => {
    ...
    message += 'Mohon konfirmasi ketersediaan ya, terima kasih! 🌸';
    
    const url = `https://wa.me/${SHOP_INFO.phone}?text=${encodeURIComponent(message)}`;
    ...
    if (favorites.length === 0) {
        container.innerHTML = `
            <div class="cart-empty">
                ...
                <p style="font-size:13px; margin-top:8px;">Tap 🤍 di produk untuk simpan~</p>
            </div>
        `;
        return;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    updateBadges();
    renderProducts();
    console.log('%c🎀 Sugarcloset loaded!', 'color: #FF8FB8; font-size: 20px; font-weight: bold;');
});
